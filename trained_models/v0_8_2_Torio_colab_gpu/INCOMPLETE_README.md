...
Source: `MyDrive/MAGI_data/trained_models/v0_8_2_Torio_colab_gpu`
(Drive folder id `1t7xABRoW9Sa4t2DwKYz7WBaJFbJJ4ZIX`, account
...
| file | note |
|---|---|
| `mix_Torio_summary.txt` | run provenance |
| `mix_Torio_task_weights.json` | all 1.0 |
| `validation/wasserstein_scores.csv` | per-variable W1 |
| `validation/correlation_diff.csv` | 5×5 Δρ matrix |
| `validation/line_integral_recovery.csv` | 4 lines, recovery + significance |

## Missing — fetch from Drive

| file | size |
|---|---|
| `mix_Torio.weights.h5` | 3.49 MB |
| `mix_Torio_quantile_transformers.joblib` | 641 KB |
| `mix_Torio_metadata.json` | 31 KB |
| `mix_Torio_config.json` | 14 KB |
| `mix_Torio_history.json` | 26 KB |
| `validation/*.png` (7 files, incl. 1.3 MB pairgrid) | ~1.7 MB |
| `validation/covariance_diff.csv` | 601 B |
...
Read from `mix_Torio_config.json` on Drive and **identical on every shared key**
...
n_types 3    4 lines    latent_dim 8    hidden [128,128,64]    beta 0.2
...
line_positions_y [-0.29158, -2.83293, -2.09660, -2.05230]
```

Training: 8,847,918 crossings, 40 epochs, 55.0 min on Colab GPU
(57.1 min including generation + validation).
