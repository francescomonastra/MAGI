...
def compute_wasserstein_scores(real_pack, gen_pack):
    return {
        ...
        "cphi_v": wasserstein_distance(
            real_pack["cphi_v_real"],
            gen_pack["cphi_v_gen"],
        ),
        "sphi_v": wasserstein_distance(
            real_pack["sphi_v_real"],
            gen_pack["sphi_v_gen"],
        ),
...
def report_generated_constraints(gen_pack, radius=100.0):
      ...
      print(
            "u_r_gen min/max:",
            gen_pack["u_r_gen"].min(),
            gen_pack["u_r_gen"].max()
      )

      print(
            "u_v_gen min/max:",
            gen_pack["u_v_gen"].min(),
            gen_pack["u_v_gen"].max()
      )
