...
## 8. Next steps (development plan summary)

Two sequenced cycles, each validated on the synthetic stress tests before a ~80 min real
CR+Small run:

1. **Cycle 1 — continuum fidelity (first):** robust/clipped `continuum_flow_y_scale` for CR
   (more spline knots on the low-E edge), and restore the Small low-E tail (more flow
   resolution/reach, and if needed sparse-tail NLL up-weighting). Acceptance: CR edge + Small
   tail sharpen, CR high-E/muon unchanged, coupling within ±0.05.
2. **Cycle 2 — line routing (second):** imbalance-aware gate loss (inverse-frequency / focal
   weighting at `model.py:3601`) + a per-line mixture-weight floor, so rare CR fluorescence
