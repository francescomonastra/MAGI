...
> **⚠ Superseded in part (2026-07-27) — every LINE number below is invalid.**
> The line-integral recovery ratios in §2, §9 and §9.1 were produced by a metric that
> (a) did not normalize for generating 1e6 events against 3.44e6/1.87e6 real ones and
> (b) used a window ~190x the pinned 4 eV line width — and against line positions taken
> from Geant4's `fluor_Bearden` table when the simulation emitted the **EADL** energies,
> so most lines were pinned 4-11 FWHM away from any real events. Corrected, the lines are
> over-generated everywhere (CR 511: 0.85 → **5.83**) rather than under-generated. See
> **`docs/v0.8.1_line_truth.md`** for the corrected table and the fixes.
> The **continuum** and **coupling** results below (§3, §4, and the coupling rows of §9)
> are unaffected — they were never measured with that metric.
...
### 9.1 Cycle 0 → 1 → 2 progression (real vs generated)

Each figure stacks the three cycles (top→bottom): **Cycle 0** = affine flow + `w_gate_aux=2`
(the run in §1–4); **Cycle 1** = + CDF warp; **Cycle 2** = + focal gate loss (γ=2). Blue =
real, orange = generated. (Cycle-0/1 checkpoints were overwritten by later runs, so these are
composited from the saved per-cycle images rather than re-generated.)

**Energy spectra** — CR: the low-E fluorescence lines and 511 grow across cycles and the
muon notch (~50 MeV) sharpens at Cycle 1; Small: the low-E tail fills in at Cycle 1 and the
lines reach ~1.0 at Cycle 2.

![CR spectrum, Cycle 0→1→2](../Plots/v0_8_cycles_CR_spectrum.png)
![Small spectrum, Cycle 0→1→2](../Plots/v0_8_cycles_Small_spectrum.png)

**Correlation triptychs** (real | generated | gen−real) — the energy↔geometry coupling is
preserved at every cycle (residuals stay within ±0.04); the fixes did not disturb it.

![CR correlation, Cycle 0→1→2](../Plots/v0_8_cycles_CR_corr.png)
![Small correlation, Cycle 0→1→2](../Plots/v0_8_cycles_Small_corr.png)
