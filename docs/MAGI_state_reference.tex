...
\midrule
$^{40}$K (Small) & \textbf{0.0134} & 0.0266 & 0.21 \\
...
\section{Cost --- measured on DM1.2}
\label{sec:cost}
...
orders of magnitude. DM1.2 is the only case where both sides ran here; the SRON
equivalent \textbf{cannot} be computed, since that reference ran on a cluster
\ev{E15}.
...
centimetres, so the gain should be far larger --- but \textbf{that has not been
measured and must not be asserted.}
...
it, a \textbf{287$\times$ amplification}, validated at the detector at
$1.118 \pm 0.373$. No amount of compute buys that from the original sample.
...
\begin{itemize}[nosep]
  ...
  \item Any numeric cost extrapolation to Athena/X-IFU --- the outer/inner
        core-hour split has not been measured on a flight mass model.
...
$^{40}$K, BuildingModel & real recorded crossings & $R = 1.118 \pm 0.373$; KS $p = 0.60$ \\
\bottomrule
\end{tabular}
\end{center}
...
ideal $1/\chi = 200$. $^{40}$K passes at matched $N$. CR sits $15$--$24\,\%$ low
and is the one open discrepancy.
...
\textbf{second design variant}. For a flight model, where the outer share far
exceeds DM1.2's $52\,\%$, break-even approaches the first redesign. Separately,
MAGI amplified an exhausted $^{40}$K sample $287\times$ ---
$1.87\times10^{6} \to 5.38\times10^{8}$ crossings --- which no amount of compute
can buy from the original sample.
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{No seed systematic is established}, in either direction. The
        previously reported $\pm40\,\%$ was an artefact of a metric that
        measures the species mix on a bimodal source; a detector-level
        three-seed CR test is running (\S\ref{sec:inprogress}).
  ...
  \item \textbf{Cost is measured on one geometry only}, and DM1.2 is the
        pessimistic case.
\end{enumerate}
