...
\caption{\textbf{Figure 2 candidate --- DM1.2 (recommended).} Crossing
...
\paragraph{Choosing Figure 2.} Three sources have a loadable v0.8.2 checkpoint.
Torio and Radio do not --- their checkpoints on this machine are incomplete
(weights still on Drive), so they cannot be generated from.
...
\noindent\textbf{Recommendation: DM1.2}, despite the worst mean residual.
$^{40}$K wins on the number, but every correlation in its truth is
...
$\Delta = +0.084$, the largest single residual anywhere.
