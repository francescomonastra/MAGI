...
feature_pack = magi.build_feature_dataframe(
    ...
    candidate_energy_lines=candidate_lines,
...
res = magi.detect_energy_lines(E, energy_bins, candidate_lines,
                               refine_bin_width_mev=4.0e-6)
...
gate_targets = magi.build_gate_targets(
    E, energy_bins, matched,
    bandwidth_mode="resolution",   # the default; see the note below
)
line_logsigma = magi.line_logsigma_from_resolution(
    [m["candidate_energy_mev"] for m in matched], resolution_ev=4.0)
\end{lstlisting}
...
118 tests covering flow round-trip identity, checkpoint save/load config
matching, gate-target construction (including the exact/resolution A/B), prior
zone-conditioning, and public API docstring coverage. They catch mechanical
regressions; they do not tell you a trained model reproduced your source.
...
# --- 3. candidate lines and the feature frame ------------------------
candidate_lines = magi.load_candidate_energy_lines(
    "CandidateLines/cryosphere_lines.json")["lines"]

feature_pack = magi.build_feature_dataframe(
    prep, energy_binning_mode="log_fixed_count", n_bins=1024, min_counts=20,
    geometry_transform="quantile_u_r_u_v_phi_r_phi_v",
    candidate_energy_lines=candidate_lines,
    refine_bin_width_mev=4.0e-6)
magi.report_feature_dataframe(feature_pack)

E           = feature_pack["filtered_prep"]["features"]["Energy"].to_numpy()
energy_bins = feature_pack["energy_bins"]

# --- 4. lines: detect, confirm doublets, build gate targets ----------
res = feature_pack["line_detection"]
matched = [m for m in res["matched_lines"] if m["count"] >= 100]
matched += magi.confirm_unresolved_candidate_lines(
    E, candidate_lines, matched, resolution_ev=4.0)

gate_targets = magi.build_gate_targets(
    E, energy_bins, matched, bandwidth_mode="exact")
