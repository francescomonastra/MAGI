...
Panel (a) DM1.2 laboratory cryostat, full deposited-energy band, counts per
          INJECTED CROSSING. Normalization-free: both sides through the same
          mass model, binary and scoring, so no Chi, exposure or area enters.
          No X-IFU sub-band is drawn - DM127 is an anti-coincidence detector
          (ACDSD in GDMLDetectorConstruction.cc), so the 0.1-30 keV X-IFU
          science band is not the relevant one for it.

Panel (b) SRON X-IFU model, Detector 1 (8x8 TES array), flux against the RUN
          CRYOAC reference. This one cannot be made normalization-free at
          spectral resolution: the reference throws Building primaries, not
          crossings, so the crossing efficiency ChiCR enters. The normalization
          recipe is copied from DataAnalysis_pkg.ipynb cell 63 rather than
          reinvented.
...
y_r = n_r / N_CROSS_REF_DM12 / wa
y_m = n_m / N_CROSS_MAGI_DM12 / wa
ey_r = np.sqrt(np.maximum(n_r, 1)) / N_CROSS_REF_DM12 / wa
ey_m = np.sqrt(np.maximum(n_m, 1)) / N_CROSS_MAGI_DM12 / wa

y_r_plot = np.where(n_r > 0, y_r, np.nan)   # empty bins -> gap, not a spike
ax.step(bins_a[:-1], y_r_plot, where="post", color=C_REF, lw=1.1,
        label=r"full Geant4 ($2\times10^{8}\ \mu$)")
ax.errorbar(cen_a, y_m, yerr=ey_m, fmt="o", ms=2.2, color=C_MAGI, lw=0,
            elinewidth=0.7, label=r"MAGI ($1.2\times10^{6}$ crossings)")
ax.set_xscale("log"); ax.set_yscale("log")
ax.set_ylabel(r"counts / injected crossing / keV")
ax.set_title("(a)  DM1.2 cryostat $-$ anti-coincidence", fontsize=8.5, loc="left")
...
axr.set_xscale("log"); axr.set_ylim(0.0, 2.0)
...
ax.text(173, 5.5e-5, "MIP", fontsize=6.5, ha="center", color="0.35")
...
ax2.errorbar(CEN_S[m], f_m[m], yerr=ef_m[m], fmt="o", ms=2.2, color=C_MAGI,
             lw=0, elinewidth=0.7, label=r"MAGI ($1.85\times10^{7}$ crossings)")
ax2.set_yscale("log")
ax2.set_ylabel(r"flux  [cts s$^{-1}$cm$^{-2}$keV$^{-1}$]")
ax2.set_title("(b)  SRON X-IFU $-$ Detector 1, TES array", fontsize=8.5, loc="left")
