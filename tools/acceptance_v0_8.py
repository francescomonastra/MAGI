...
def rebuild_pipeline(name, candidate_lines):
...
for name in args.sources:
    log("=" * 62)
    log(f"SOURCE: {name}")
    save_dir = f"trained_models/v0_8_{name}{args.save_dir_suffix}"
    ...
    feature_pack, dataset_pack, matched = rebuild_pipeline(name, candidate_lines)
